@if(count($employees) > 0)
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end pt-4 pb-8">
        ...
        <div class="order-2 md:order-1 bg-white rounded-[32px] p-6 border border-slate-200 shadow-md card-3d relative overflow-hidden flex flex-col items-center text-center transform md:scale-95 md:-translate-y-4">
            ...
            <div class="w-24 h-24 rounded-full bg-slate-100 mb-4 overflow-hidden border-4 border-white shadow-lg relative">
                <img src="{{ $employees[1]->employee->photo }}" alt="Profile" class="w-full h-full object-cover" onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($employees[1]->employee->full_name) }}&background=F1F5F9&color=64748B'">
            </div>
            <h3 class="text-sm font-black text-slate-900 mb-1 leading-tight">{{ $employees[1]->employee->full_name }}</h3>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">{{ $employees[1]->employee->work_unit->name ?? 'Staf' }}</p>
            <div class="w-full bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-2">
                <div class="flex justify-between items-center text-xs font-bold">
                    ...
                    <span class="text-slate-900">{{ $employees[1]->score }} Poin</span>
                </div>
                <div class="flex justify-between items-center text-[10px] font-semibold text-slate-400">
                    <span>Hadir: {{ $employees[1]->total_present }}x</span>
                    <span>Telat: {{ $employees[1]->late_count }}x</span>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="order-1 md:order-2 bg-gradient-to-b from-amber-500 to-amber-600 rounded-[40px] p-8 border border-amber-400 shadow-2xl shadow-amber-500/20 card-3d relative overflow-hidden flex flex-col items-center text-center z-10">
            ...
            <div class="w-32 h-32 rounded-full bg-white mb-5 overflow-hidden border-4 border-amber-300 shadow-xl relative z-10">
                <img src="{{ $employees[0]->employee->photo }}" alt="Profile" class="w-full h-full object-cover" onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($employees[0]->employee->full_name) }}&background=FFFBEB&color=B45309'">
            </div>
            <h3 class="text-lg font-black text-white mb-1 leading-tight relative z-10">{{ $employees[0]->employee->full_name }}</h3>
            <p class="text-[10px] font-bold text-amber-100 uppercase tracking-widest mb-6 relative z-10">{{ $employees[0]->employee->work_unit->name ?? 'Staf' }}</p>
            <div class="w-full bg-black/10 rounded-3xl p-5 backdrop-blur-sm border border-white/10 space-y-3 relative z-10">
                <div class="flex justify-between items-center text-sm font-black text-white">
                    ...
                    <span class="text-xl">{{ $employees[0]->score }} Poin</span>
                </div>
                <div class="h-px w-full bg-white/20"></div>
                <div class="flex justify-between items-center text-[11px] font-bold text-amber-100 uppercase">
                    <span class="flex items-center gap-1"><i data-lucide="check-circle" class="w-3 h-3"></i> {{ $employees[0]->total_present }} Hadir</span>
                    <span class="flex items-center gap-1"><i data-lucide="clock" class="w-3 h-3"></i> {{ $employees[0]->late_count }} Telat</span>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="order-3 md:order-3 bg-white rounded-[32px] p-6 border border-slate-200 shadow-md card-3d relative overflow-hidden flex flex-col items-center text-center transform md:scale-95 md:-translate-y-4">
            ...
            <div class="w-24 h-24 rounded-full bg-slate-100 mb-4 overflow-hidden border-4 border-white shadow-lg relative">
                <img src="{{ $employees[2]->employee->photo }}" alt="Profile" class="w-full h-full object-cover" onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($employees[2]->employee->full_name) }}&background=FFF7ED&color=C2410C'">
            </div>
            <h3 class="text-sm font-black text-slate-900 mb-1 leading-tight">{{ $employees[2]->employee->full_name }}</h3>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">{{ $employees[2]->employee->work_unit->name ?? 'Staf' }}</p>
            <div class="w-full bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-2">
                <div class="flex justify-between items-center text-xs font-bold">
                    <span class="text-slate-500">Skor</span>
                    <span class="text-slate-900">{{ $employees[2]->score }} Poin</span>
                </div>
                <div class="flex justify-between items-center text-[10px] font-semibold text-slate-400">
                    <span>Hadir: {{ $employees[2]->total_present }}x</span>
                    <span>Telat: {{ $employees[2]->late_count }}x</span>
                </div>
            </div>
        </div>
        @endif
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden card-3d">
        <div class="px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
            ...
            <h4 class="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Peringkat 4 - {{ count($employees) }}</h4>
        </div>
