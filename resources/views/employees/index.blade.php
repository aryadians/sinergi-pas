...
<!-- Edit Modal -->
<div id="editModal" class="fixed inset-0 bg-black/70 hidden flex items-center justify-center z-50 p-6 backdrop-blur-xl">
    <div class="bg-white w-full max-w-4xl rounded-[64px] shadow-2xl animate-in zoom-in duration-300 overflow-hidden border border-[#EFEFEF]">
        <div class="bg-[#1E2432] p-12 text-white flex justify-between items-center relative">
            <div class="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>
            <div class="relative">
                <h3 class="text-3xl font-black italic tracking-tight">Profil Management</h3>
                <p class="text-[10px] font-black opacity-60 uppercase tracking-[0.4em] mt-2">Modifikasi Hak Akses & Data Pegawai</p>
            </div>
            <button type="button" onclick="document.getElementById('editModal').classList.add('hidden')" class="relative w-14 h-14 flex items-center justify-center rounded-2xl bg-white/10 hover:bg-white/20 transition-all border border-white/20">
                <i data-lucide="x" class="w-6 h-6"></i>
            </button>
        </div>

        <form id="editForm" method="POST" enctype="multipart/form-data" class="p-14">
            @csrf @method('PUT')
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-14 gap-y-10">
                <div class="space-y-8">
                    <div class="flex items-center gap-4 mb-4">
                        <div class="w-10 h-10 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 shadow-sm border border-blue-100">
                            <i data-lucide="user" class="w-5 h-5"></i>
                        </div>
                        <h4 class="text-xs font-black text-[#1E2432] uppercase tracking-[0.3em] italic">Informasi Pokok</h4>
                    </div>
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-[#8A8A8A] uppercase tracking-[0.2em] ml-2">Nama Lengkap</label>
                        <input type="text" name="full_name" id="edit_full_name" required class="w-full px-8 py-5 rounded-[28px] border border-[#EFEFEF] bg-[#FCFBF9] text-sm font-bold text-[#1E2432] focus:ring-12 focus:ring-red-500/5 focus:border-[#E85A4F] outline-none transition-all shadow-inner">
                    </div>
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-[#8A8A8A] uppercase tracking-[0.2em] ml-2">Identitas NIP</label>
                        <input type="text" name="nip" id="edit_nip" required class="w-full px-8 py-5 rounded-[28px] border border-[#EFEFEF] bg-[#FCFBF9] text-sm font-bold text-[#1E2432] focus:ring-12 focus:ring-red-500/5 focus:border-[#E85A4F] outline-none transition-all shadow-inner">
                    </div>
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-[#8A8A8A] uppercase tracking-[0.2em] ml-2">Jabatan</label>
                        <div class="relative">
                            <select name="position_id" id="edit_position_id" required class="w-full px-8 py-5 rounded-[28px] border border-[#EFEFEF] bg-[#FCFBF9] text-sm font-bold outline-none focus:ring-12 focus:ring-red-500/5 focus:border-[#E85A4F] transition-all appearance-none cursor-pointer shadow-inner">
                                @foreach($positions as $pos)
                                    <option value="{{ $pos->id }}">{{ $pos->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-[#8A8A8A] uppercase tracking-[0.2em] ml-2">Unit Kerja</label>
                        <div class="relative">
                            <select name="work_unit_id" id="edit_work_unit_id" required class="w-full px-8 py-5 rounded-[28px] border border-[#EFEFEF] bg-[#FCFBF9] text-sm font-bold outline-none focus:ring-12 focus:ring-red-500/5 focus:border-[#E85A4F] transition-all appearance-none cursor-pointer shadow-inner">
                                @foreach($workUnits as $unit)
                                    <option value="{{ $unit->id }}">{{ $unit->name }}</option>
                                @endforeach
                            </select>
                ...
                <div class="space-y-8">
                    <div class="flex items-center gap-4 mb-4">
                        <div class="w-10 h-10 bg-green-50 rounded-2xl flex items-center justify-center text-green-600 shadow-sm border border-green-100">
                            <i data-lucide="shield-check" class="w-5 h-5"></i>
                        </div>
                        <h4 class="text-xs font-black text-[#1E2432] uppercase tracking-[0.3em] italic">Keamanan & Media</h4>
                    </div>
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-[#8A8A8A] uppercase tracking-[0.2em] ml-2">Email Aktif</label>
                        <input type="email" name="email" id="edit_email" required class="w-full px-8 py-5 rounded-[28px] border border-[#EFEFEF] bg-[#FCFBF9] text-sm font-bold text-[#1E2432] focus:ring-12 focus:ring-red-500/5 focus:border-[#E85A4F] outline-none transition-all shadow-inner">
                    </div>
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-[#8A8A8A] uppercase tracking-[0.2em] ml-2">Reset Password (Opsional)</label>
                        <input type="password" name="password" placeholder="Kosongkan jika tidak ada perubahan" class="w-full px-8 py-5 rounded-[28px] border border-[#EFEFEF] bg-[#FCFBF9] text-sm font-bold text-[#1E2432] focus:ring-12 focus:ring-red-500/5 focus:border-[#E85A4F] outline-none transition-all shadow-inner">
                    </div>
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-[#8A8A8A] uppercase tracking-[0.2em] ml-2">Foto Profil</label>
                        <div class="flex gap-4">
                            <label class="flex-1 relative group cursor-pointer">
                                ...
                                <div class="w-full px-8 py-5 rounded-[28px] border-2 border-dashed border-[#EFEFEF] bg-[#FCFBF9] text-[10px] font-black text-[#8A8A8A] uppercase text-center group-hover:border-[#E85A4F] group-hover:bg-red-50/30 transition-all shadow-inner flex items-center justify-center gap-3">
                                    <i data-lucide="image" class="w-4 h-4"></i> Pilih Gambar
                                </div>
                            </label>
                            <button type="button" id="btnDeleteEmployeePhoto" onclick="confirmDeleteEmployeePhoto()" class="hidden w-16 h-16 bg-red-50 text-red-500 rounded-[28px] border border-red-100 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center shadow-lg shadow-red-100 action-btn" title="Hapus Foto">
            ...
            <div class="mt-16 flex flex-col md:flex-row gap-6">
                <button type="submit" class="flex-1 bg-[#E85A4F] text-white py-6 rounded-[32px] font-black text-lg hover:bg-[#d44d42] transition-all shadow-2xl shadow-red-200 active:scale-[0.98] flex items-center justify-center gap-4 group">
                    Perbarui Data Pegawai <i data-lucide="save" class="w-6 h-6 group-hover:scale-110 transition-transform"></i>
                </button>
                <button type="button" onclick="document.getElementById('editModal').classList.add('hidden')" class="px-14 py-6 rounded-[32px] border border-[#EFEFEF] text-[#8A8A8A] font-black text-[10px] uppercase tracking-[0.3em] hover:bg-gray-100 transition-all">
                    Batalkan
                </button>
            </div>
        </form>
...
<script>
    ...
    function openEditModal(employee, email) {
        ...
        if (employee.photo) btnDelete.classList.remove('hidden'); else btnDelete.classList.add('hidden');
