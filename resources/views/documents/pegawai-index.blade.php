...
<div class="mb-12">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        <div>
            <h2 class="text-3xl font-black text-[#1E2432] tracking-tight mb-2">Arsip Digital Anda</h2>
            <p class="text-sm text-[#8A8A8A] font-medium">Halo, <span class="text-[#E85A4F] font-bold">{{ auth()->user()->name }}</span>. Kelola dan akses seluruh dokumen kepegawaian Anda di sini.</p>
        </div>
        
        <button onclick="document.getElementById('uploadModal').classList.remove('hidden')" 
            class="bg-[#E85A4F] text-white px-10 py-4 rounded-[24px] font-black hover:bg-[#d44d42] transition-all flex items-center gap-3 shadow-xl shadow-red-100 active:scale-95 group">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-[32px] border border-[#EFEFEF] shadow-sm">
            ...
            <h3 class="text-2xl font-black text-[#1E2432]">{{ $documents->count() }} <span class="text-xs text-[#ABABAB] font-bold uppercase ml-1">Berkas</span></h3>
        </div>
        <div class="bg-white p-6 rounded-[32px] border border-[#EFEFEF] shadow-sm">
        ...
        <div class="bg-white p-6 rounded-[32px] border border-[#EFEFEF] shadow-sm">
            ...
            <h3 class="text-sm font-black text-[#1E2432] mt-2">{{ $documents->first() ? $documents->first()->created_at->diffForHumans() : '-' }}</h3>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8">
    ...
    <div class="group bg-white p-8 rounded-[48px] border border-[#EFEFEF] hover:border-[#E85A4F] hover:shadow-2xl hover:shadow-red-100/30 transition-all duration-500 flex flex-col justify-between h-[320px] relative overflow-hidden">
        <!-- Status Badge -->
        ...
        <div>
            <div class="w-16 h-16 bg-[#F5F4F2] rounded-3xl flex items-center justify-center text-[#8A8A8A] group-hover:bg-[#E85A4F] group-hover:text-white transition-all duration-500 mb-6 shadow-inner">
            ...
            <h4 class="text-xl font-black text-[#1E2432] leading-tight group-hover:text-[#E85A4F] transition-colors line-clamp-2">{{ $doc->title }}</h4>
        ...
        <div class="flex items-center justify-between mt-auto pt-6 border-t border-dashed border-[#EFEFEF]">
            <div class="flex gap-2">
                <button onclick="openPreview('{{ route('documents.preview', $doc->id) }}', '{{ $doc->title }}', '{{ $doc->file_path }}')" class="bg-[#FCFBF9] p-3 rounded-2xl text-[#1E2432] hover:bg-[#1E2432] hover:text-white transition-all shadow-sm" title="Pratinjau">
                    <i data-lucide="eye" class="w-4 h-4"></i>
                </button>
                <button onclick="openRevisionModal({{ $doc->id }}, '{{ $doc->title }}')" class="bg-[#FCFBF9] p-3 rounded-2xl text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="Revisi">
            ...
            <div class="flex gap-2">
                @if(!$doc->is_locked)
                    <a href="{{ route('documents.download', $doc->id) }}" target="_blank" class="bg-[#FCFBF9] p-3 rounded-2xl text-[#E85A4F] hover:bg-[#E85A4F] hover:text-white transition-all no-loader shadow-sm" title="Unduh">
                        <i data-lucide="download" class="w-4 h-4"></i>
                    </a>
                    <form id="deleteDoc-{{ $doc->id }}" action="{{ route('documents.destroy', $doc->id) }}" method="POST" class="no-loader">
                        @csrf @method('DELETE')
                        <button type="button" onclick="confirmDocDelete({{ $doc->id }})" class="bg-[#FCFBF9] p-3 rounded-2xl text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-sm" title="Hapus">
    ...
    <div class="col-span-full bg-white rounded-[48px] p-20 border border-dashed border-[#EFEFEF] text-center">
        <div class="w-24 h-24 bg-[#FCFBF9] rounded-[32px] flex items-center justify-center mx-auto mb-6">
            <i data-lucide="folder-open" class="w-10 h-10 text-[#ABABAB]"></i>
        </div>
        <h3 class="text-2xl font-black text-[#1E2432] mb-2">Belum Ada Dokumen</h3>
        <p class="text-sm text-[#8A8A8A] font-medium max-w-xs mx-auto mb-8">Anda belum mengunggah dokumen apapun ke dalam sistem.</p>
        <button onclick="document.getElementById('uploadModal').classList.remove('hidden')" class="bg-[#1E2432] text-white px-8 py-4 rounded-2xl font-black hover:bg-[#E85A4F] transition-all shadow-xl">
            Unggah Sekarang
        </button>
    </div>
    @endforelse
...
<div id="uploadModal" class="fixed inset-0 bg-black/60 hidden flex items-center justify-center z-50 p-6 backdrop-blur-md">
    <div class="bg-white w-full max-w-lg rounded-[48px] p-12 shadow-2xl animate-in zoom-in duration-300">
        <div class="flex justify-between items-center mb-10">
            <div>
                <h3 class="text-2xl font-black text-[#1E2432] tracking-tight">Unggah Arsip</h3>
        ...
        <form action="{{ route('documents.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="space-y-4">
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-[#1E2432] uppercase tracking-[0.2em] ml-1">Nama Dokumen</label>
                    <input type="text" name="title" required placeholder="Contoh: SK CPNS 2024" 
                        class="w-full px-6 py-4 rounded-[20px] border border-[#EFEFEF] bg-[#FCFBF9] text-sm font-bold outline-none focus:ring-4 focus:ring-red-500/5 transition-all">
                </div>
                
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-[#1E2432] uppercase tracking-[0.2em] ml-1">Kategori</label>
                    <select name="document_category_id" required class="w-full px-6 py-4 rounded-[20px] border border-[#EFEFEF] bg-[#FCFBF9] text-sm font-bold outline-none focus:ring-4 focus:ring-red-500/5 transition-all appearance-none cursor-pointer">
                ...
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-[#1E2432] uppercase tracking-[0.2em] ml-1">Berkas Dokumen</label>
                    <div class="relative group">
                        <input type="file" name="file" required class="w-full px-6 py-12 rounded-[32px] border-2 border-dashed border-[#EFEFEF] bg-[#FCFBF9] text-xs font-bold text-[#8A8A8A] file:hidden cursor-pointer hover:border-[#E85A4F] transition-all text-center">
                        <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none opacity-40 group-hover:opacity-100 transition-opacity">
                            <i data-lucide="file-up" class="w-10 h-10 text-[#E85A4F] mb-2"></i>
...
<!-- Preview Modal -->
<div id="previewModal" class="fixed inset-0 bg-[#1E2432]/95 hidden flex items-center justify-center z-[100] p-6 md:p-12 backdrop-blur-xl">
    <div class="bg-white w-full h-full max-w-7xl rounded-[56px] overflow-hidden flex flex-col shadow-2xl relative">
        <div class="p-8 border-b border-[#EFEFEF] flex justify-between items-center bg-white">
            <div>
                <h3 id="previewTitle" class="text-2xl font-black text-[#1E2432]">Pratinjau</h3>
                <p class="text-[10px] font-black text-[#8A8A8A] uppercase tracking-widest mt-1">Sistem Keamanan Sinergi PAS</p>
            </div>
            <button onclick="document.getElementById('previewModal').classList.add('hidden')" class="bg-[#FCFBF9] p-4 rounded-[24px] shadow-sm border border-[#EFEFEF] hover:bg-red-50 hover:text-red-500 transition-all group">
                <i data-lucide="x" class="w-7 h-7 group-hover:rotate-90 transition-transform"></i>
            </button>
        </div>
        <div class="flex-1 bg-[#F5F4F2] overflow-auto flex items-center justify-center p-6 md:p-12 relative" id="previewContent">
...
<div id="revisionModal" class="fixed inset-0 bg-black/60 hidden flex items-center justify-center z-50 p-6 backdrop-blur-md">
    <div class="bg-white w-full max-w-md rounded-[48px] p-12 shadow-2xl animate-in zoom-in duration-300">
        <div class="flex justify-between items-center mb-10">
            <div>
                <h3 class="text-2xl font-black text-[#1E2432]">Pembaruan</h3>
                <p class="text-[10px] font-bold text-[#8A8A8A] uppercase tracking-widest mt-1">Kirim Revisi Dokumen</p>
            </div>
            <button onclick="document.getElementById('revisionModal').classList.add('hidden')" class="bg-[#FCFBF9] p-3 rounded-2xl text-[#8A8A8A] hover:text-red-500 transition-all border border-[#EFEFEF]">
                <i data-lucide="x" class="w-6 h-6"></i>
            </button>
        </div>
        <form id="revisionForm" action="" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="space-y-4">
                <div class="bg-[#FCFBF9] p-6 rounded-3xl border border-[#EFEFEF]">
                    <p class="text-[10px] font-black text-[#ABABAB] uppercase tracking-widest mb-2">Merevisi Berkas:</p>
                    <p id="revision_doc_title" class="text-sm font-black text-[#E85A4F]"></p>
                </div>
                <div class="relative group">
                    <input type="file" name="file" required class="w-full px-6 py-12 rounded-[32px] border-2 border-dashed border-[#EFEFEF] bg-[#FCFBF9] text-xs font-bold text-[#8A8A8A] file:hidden cursor-pointer hover:border-blue-500 transition-all text-center">
                    <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none opacity-40 group-hover:opacity-100 transition-opacity">
            ...
            <button type="submit" class="w-full bg-[#1E2432] text-white py-5 rounded-[28px] font-black text-lg hover:bg-blue-600 transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3">
