...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        <a href="{{ route('dashboard.export.pdf') }}" download="laporan-operasional.pdf" class="p-6 rounded-[32px] bg-white border border-slate-200 shadow-sm hover:border-red-500 hover:shadow-xl transition-all group flex flex-col items-center text-center gap-3 card-3d no-loader">
...
<script>
    window.addEventListener('DOMContentLoaded', () => {
        ...
        new Chart(document.getElementById('latenessChart'), {
            type: 'line',
            data: {
                labels: {!! json_encode($disciplinaryData['latenessTrend']['labels']) !!},
                datasets: [{
                    label: 'Jumlah Keterlambatan',
                    data: {!! json_encode($disciplinaryData['latenessTrend']['values']) !!},
                    borderColor: '#f59e0b',
                    fill: true,
